...
class PeThemeContent {
	...
	public function filter($tax,$sep = "",$aclass="label",$template = '%s') {
		...
		if (is_array($terms) && count($terms) > 0) {
			$buffer = apply_filters("pe_theme_filter_item",sprintf('<a class="%s active" data-category="" href="#">%s</a>%s',$aclass,__("todo",'Pixelentity Theme/Plugin'),"$sep\n"),$aclass,"",__("todo",'Pixelentity Theme/Plugin'));
